...
public static class TextBoxAssist
{
        ...
        DependencyPropertyChangedEventArgs eventArgs)
    {
        ...
        if (newValue != textBox.CaretIndex) textBox.CaretIndex = newValue < 0 ? 0 : newValue;
    }

    private static void SelectionChangedForCaretIndex(object sender, RoutedEventArgs eventArgs)
    {
        if (sender is TextBox textBox) SetCaretIndex(textBox, textBox.CaretIndex);
    }

    private static void SelectionChangedForSelectionLength(object sender, RoutedEventArgs eventArgs)
    {
        if (sender is TextBox textBox) SetSelectionLength(textBox, textBox.SelectionLength);
    }
        ...
        DependencyPropertyChangedEventArgs eventArgs)
    {
        ...
        if (newValue != textBox.SelectionLength) textBox.SelectionLength = newValue < 0 ? 0 : newValue;
    }
}