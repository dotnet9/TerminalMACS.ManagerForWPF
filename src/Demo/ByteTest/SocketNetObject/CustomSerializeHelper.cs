...
public static class CustomSerializeHelper
{
    ...
    public static byte[] Serialize<T>(T data, long systemId)
    {
        if (data == null)
        {
        ...
        var netObjectInfo = SerializeHelper.GetNetObjectHeadAttribute(data.GetType());
        var bodyBuffer = Serialize(data);
        using var stream = new MemoryStream();
        using var writer = new BinaryWriter(stream, SerializeHelper.DefaultEncoding);

        writer.Write(SerializeHelper.PacketHeadLen + bodyBuffer.Length);
    ...
    {
        var bodyBufferLen = buffer.Length - SerializeHelper.PacketHeadLen;
        using var stream = new MemoryStream(buffer, SerializeHelper.PacketHeadLen,
            bodyBufferLen);
